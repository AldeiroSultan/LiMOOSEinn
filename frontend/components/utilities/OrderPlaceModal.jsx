...
function OrderPlaceModal({ bookingModal, setBookingModal }) {
  ...
  // function to handle placed booking order
  const handlePlacedOrder = () => {
    if (selectedDates.length === 0) {
      notificationWithIcon('error', 'ERROR', 'Minimum 1 date selection is required to placed an vehicle booking order.');
    } else if (selectedDates.length > 5) {
      notificationWithIcon('error', 'ERROR', 'Maximum 5 days selection possible to placed an vehicle booking order.');
    } else {
      confirm({
        title: 'Are your selected dates booked this Vehicle?',
        icon: <ExclamationCircleOutlined />,
        okText: 'Ok',
        cancelText: 'Cancel',
        onOk() {
          return new Promise((resolve, reject) => {
            ApiService.post(`/api/v1/placed-booking-order/${bookingModal?.vehicleId}`, {
              booking_dates: selectedDates
            })
              .then((res) => {
                resolve();
                if (res?.result_code === 0) {
                  notificationWithIcon('success', 'SUCCESS', (res?.result?.message || 'Your vehicle booking order placed successful'));
                  setBookingModal((prevState) => ({ ...prevState, open: false, vehicleId: null }));
  ...
  return (
    <Modal
      title='Select data which dates are you Booked Vehicle:'
      open={bookingModal.open}
      onOk={() => setBookingModal((prevState) => (
        { ...prevState, open: false, vehicleId: null }
      ))}
...
OrderPlaceModal.defaultProps = {
  bookingModal: { open: false, vehicleId: null }
};
