...
exports.vehicleReviewAdd = async (req, res) => {
  try {
    ...
    const newReview = new Review({
      user_id: req.user.id,
      vehicle_id: myBooking.vehicle_id,
    ...
    res.status(201).json(successResponse(
      0,
      'SUCCESS',
      'Your vehicle booking order placed successful',
      savedReview
    ));
  } catch (error) {
...
exports.getVehicleReviewsList = async (req, res) => {
  try {
    // finding by a review by id
    let myReviews = null;

    if (/^[0-9a-fA-F]{24}$/.test(req.params.vehicle_id)) {
      myReviews = await Review.find({ vehicle_id: req.params.vehicle_id })
        .populate('user_id');
    } else {
      return res.status(400).json(errorResponse(
        1,
    ...
    const reviewQuery = new MyQueryHelper(Review.find({ vehicle_id: req.params.vehicle_id })
...
exports.editSelfVehicleReview = async (req, res) => {
  try {
    const { rating, message } = req.body;
    ...
    if (myReviews?.user_id?.id !== req?.user?.id) {
      return res.status(406).json(errorResponse(
        6,
        'UNABLE TO ACCESS',
        'Sorry! You can update only self vehicle reviews'
      ));
    }
    ...
    res.status(200).json(successResponse(
      0,
      'SUCCESS',
      'Your vehicle review update successful',
      updatedReview
    ));
  } catch (error) {
    res.status(500).json(errorResponse(
