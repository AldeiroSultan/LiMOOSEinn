...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    const availableVehicles = await Vehicle.find({ vehicle_status: 'available' });
    const unavailableVehicles = await Vehicle.find({ vehicle_status: 'unavailable' });
    const bookedVehicles = await Vehicle.find({ vehicle_status: 'booked' });
    ...
    res.status(200).json(successResponse(
      ...
      'Dashboard information get successful',
      {
        ...
        vehicles_info: {
          total_vehicles: totalVehicles?.length || 0,
          available_vehicles: availableVehicles?.length || 0,
          unavailable_vehicles: unavailableVehicles?.length || 0,
          booked_vehicles: bookedVehicles?.length || 0
        },
        booking_info: {
